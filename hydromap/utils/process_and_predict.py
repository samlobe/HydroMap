...
try:
    from hydromap.utils.get_PCs import get_PCs
except ImportError:
    try:
        from get_PCs import get_PCs
    except ImportError:
        sys.exit("ERROR: Cannot import get_PCs.py.")
...
def residue_key(res):
    """Return sort key, residue token, canonical chain token, and diagnostics."""
    return residue_sort_token_chain(res)


def residue_scoped_path(directory, prot, token, chain, suffix):
    if chain:
        return f'{directory}/{prot}_res{token}_chain{chain}_{suffix}'
    return f'{directory}/{prot}_res{token}_{suffix}'


def residue_scoped_missing_message(path, alternates):
    extra = ""
    if alternates:
        extra = (
            f" Alternate segment token(s) {alternates} were observed in the input, "
            "but HydroMap uses PDB chainID for residue-scoped outputs."
        )
    return f"{path} missing.{extra}"

def build_residue_records(u, multi_chain=False):
    """
    Enumerate residues in a deterministic order and return a list of dicts:
      {
        'sel':   "resid 112A and chainID A"  (or "resid 112A" if not multi_chain)
        'token': "112A",
        'chain': "A" or "",
        'alternate_chains': ["B", ...]  # diagnostic segid values, not lookup tokens
      }
    """
    ...
    recs = []
    for r in residues:
        _, token, chain, alternates = residue_key(r)
        sel = f"resid {token} and chainID {chain}" if multi_chain and chain else f"resid {token}"
        recs.append({"sel": sel, "token": token, "chain": chain, "alternate_chains": alternates})
    return recs
...
def main():
    ...
    if model_name == 'Fdewet.joblib' and a.forcefield != 'a99SBdisp':
        sys.exit('Fdewet.joblib requires forcefield=a99SBdisp.')
    
    if model_name == 'Fdewet_isolated_aa_multi_forcefield.joblib' and a.forcefield not in ['a99SBdisp', 'a03ws','C36m']:
        sys.exit('Fdewet_isolated_aa_multi_forcefield.joblib supports a99SBdisp, a03ws, and C36m.')
    ...
    for i,group in enumerate(tqdm(groups,desc='Angles')):
        ...
        if not os.path.exists(path):
            warnings.warn(residue_scoped_missing_message(path, recs[i]["alternate_chains"] if not a.groupsFile else [])); continue
    ...
    for i,group in enumerate(tqdm(groups_kept,desc='Potentials')):
        if a.groupsFile:
            ...
            token = recs_kept[i]["token"]
            if a.multiChain:
                pot = residue_scoped_path(
                    a.potentialsDir,
                    prot,
                    token,
                    recs_kept[i]["chain"],
        ...
        if not os.path.exists(pot):
            warnings.warn(residue_scoped_missing_message(pot, recs_kept[i]["alternate_chains"] if not a.groupsFile else [])); continue
        df=pd.read_csv(pot)
    ...
    res=pd.DataFrame({'MDAnalysis_selection_strings':features['MDAnalysis_selection_strings'],
                      ...
                      '40-50_tri':features['40-50_tri'],
                      '140-150_tri':features['140-150_tri']
                      })
