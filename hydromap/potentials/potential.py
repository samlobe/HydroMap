...
Example
-------
python potential.py ../protein_processed.pdb ../traj.dcd --top ../protein.xml -res 10 -t 5 --skip 50
"""
...
def residue_selection(chain_id: str, res_token: str):
    """
    Build an MDAnalysis selection string for a residue, including insertion code if present.
    If chain_id is None/empty, omit it from the selection. (No heavy-atom filter here.)
    """
...
# ----------------------------------------------------------------------
def prepare_system(system, group_idx, solvent_idx, cutoff_nm=0.425):
    """
...
def main():
    ...
    if args.selection:
        ...
        sel_str, _, _ = residue_selection(args.chain, args.resid)
    ...
    solvent_atoms = u.select_atoms(NORMALIZED_WATER_SELECTION)
    ...
    system = load_openmm_system(args.top, pdb_path)
    system = prepare_system(system, group_idx, solvent_idx, cutoff_nm=args.cutoff/10)
    ...
    for ts in tqdm(u.trajectory[first::args.skip]):
        ...
        water_oxygens = NORMALIZED_WATER_OXYGEN_SELECTION
        group = sel_str
